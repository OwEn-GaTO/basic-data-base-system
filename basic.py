import sys,os
class basicdb:
    def write_row(self,name,row,array):
        for i in range(1,self.sizeY(name)+1):
            self.write_cordinate(name,array[i - 1],i,row)
    def write_colum(self,name,colum,array):

        for i in range(1,self.sizeY(name)+1):
            self.write_cordinate(name,array[i - 1],colum,i)
